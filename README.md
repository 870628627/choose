...
一个家庭自用的多资产研究、对比和复盘 MVP。A 股自选池支持自动保存基础信息、日频估值行情、财务指标、公告标题，并生成研究评分和风险标签。

本项目允许生成荐股观点、交易方案、目标价和涨跌判断，并在网页端接入 TradingAgents 中文交易报告。首页可直接输入 `600519`、`NVDA`、`BTC-USD` 等资产符号生成全球资产报告。

## 技术栈

- 前端：React + Vite + TypeScript + Tailwind CSS
- 后端：Node.js + Express + SQLite
- 数据采集：Python data-worker；A 股自选数据优先 AKShare，TradingAgents A 股行情优先东方财富
...
1. 在首页“全球资产 TradingAgents 报告”输入 `600519`、`NVDA` 或 `BTC-USD` 生成中文交易研究报告。
2. 在 A 股自选池输入 `600519`、`000001` 或 `300750` 添加股票。
3. 到“同步管理”点击“同步全部自选股”。
4. 打开自选详情页查看自动数据、研究评分、风险标签，并生成或导出 TradingAgents 中文交易报告。
5. 到“自选对比”选择 2-4 只股票横向对比。
6. 到“风险排雷”查看所有股票风险标签。
7. 到“复盘”记录当时判断和后续结果。
